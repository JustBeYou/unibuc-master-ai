...
@testset "Hard binary classification: two moons - gradient training" begin
    X, _, test_X, y, _, test_y = BayesianNetworks.HardBinaryClassification.make_dataset(5000)
...
@testset "Hard binary classification: two moons - MCMC inference" begin
    X, _, test_X, y, _, test_y = BayesianNetworks.HardBinaryClassification.make_dataset(1000)
    ...
    @test BayesianNetworks.accuracy(y, mean_y_pred) > 0.80
    ...
    @test BayesianNetworks.accuracy(test_y, mean_test_y_pred) > 0.80
end